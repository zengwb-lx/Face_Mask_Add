...
class FaceMasker:
    ...
    def __init__(self, is_aug):
        ...
        self.prn = PRN('models/prnet_pytorch.pth')
    ...
    def add_mask_two(self, image, face_lms, template_name, masked_face_path):
        ...
        if c == 4:
            image = image[:,:,:3]
        # for face_lm in face_lms:
        pos, vertices = self.get_vertices(face_lms[0], image)  # 3d reconstruction -> get texture.
        image = image/255. #!!
        ...
        new_image = image * (1 - face_mask[:, :, np.newaxis]) + new_image * face_mask[:, :, np.newaxis]
        new_image = np.clip(new_image, -1, 1)  # must clip to (-1, 1)!
        imsave('222.png', new_image)

        exit()
        return new_image

    def add_mask_one(self, image, face_lms, template_name, masked_face_path):
        """Add mask to one image.
    ...
    def get_vertices(self, face_lms, image):
        ...
        # vertices_ = []
        # for face_lm in face_lms:
        #     lms_info = read_info.read_landmark_106_array(face_lm)
        #     pos = self.prn.process(image, lms_info)
        #     vertices = self.prn.get_vertices(pos)
        #     pos_.append(pos)
        #     vertices_.append(vertices)
        #     print(pos.shape, vertices.shape)
        # return pos_, vertices_
